...
def predict_missing_values(star_data, mse_threshold=0.05,number=1500):
    good_stars = {}
...
def sigma_clip_windowed(data, window_size=50, sigma_threshold=3):
...
for filename in os.listdir(input_dir):
    ...
    with tqdm(total=len(batch_data), desc="Processing Batch") as pbar:
        ...
        for star_id, data in batch_data.items():
            ...
            sap_flux[missing_indices] =0
            # if Period_day>5 or Period_day>35:  #prot 5,25
            if Period_day<0 or Period_day>70:
