...
def training(model, device, n_stop, tol, optimizer, loss_func, n_epochs, \
    ...
    early_stopped = False
    no_improve = 0
    ProgressBar=tqdm(range(n_epochs), desc='Training', unit='epoch', leave=False)
    # Wrap the training loop with tqdm to create a progress bar
    for epoch in ProgressBar:
        ...
        with torch.no_grad():
            for nval, (xval, sval, yval) in enumerate(val_loader):
                ...
                running_loss[0,epoch] += loss_func(valpred, yval).item()
        ...
        if epoch > 0:
            if losses[0,epoch] <= np.min(losses[0,:epoch]):
                ...
                model.log = None
                torch.save(model, save_path + 'model_complete.pth')
                model.log = log            
        ...
        for n, (x, s, y) in enumerate(train_loader):
            ...
            running_loss[1,epoch] += loss.item()
        losses[1,epoch] = 1e5 * (running_loss[1,epoch] / n_train)

        # Update the progress bar with the current losses

        ProgressBar.set_description(f"Training (Val Loss: {losses[0,epoch]:.5f},Train Loss: {losses[1,epoch]:.5f})")
